...
export class SubscriberContainer extends React.Component {
  ...
  render() {
    ...
    const assignHostRef = host => {
      this.host_field = host;
    };
    const assignLicenseRef = license => {
      this.license_field = license;
    };
    const assignStreamNameRef = streamName => {
      this.stream_name_field = streamName;
    };
    const assignUsernameRef = username => {
      this.username_field = username;
    };
    const assignPasswordRef = password => {
      this.password_field = password;
    };

    if (this.state.hasPermissions && this.state.hasStarted) {
      // if (this.state.isPublisher) {
      //   return <Publisher streamProps={this.state.streamProps} style={styles.container} onStop={this.onStop} />;
      // }

      return <Subscriber streamProps={this.state.streamProps} style={styles.container} onStop={this.onStop} />;
    }

    return (
      <View style={styles.container}>
        <View style={styles.formField}>
          <TextInput ref={assignHostRef.bind(this)} {...this.state.hostFieldProps} onChangeText={this.onHostChange} />
        </View>
        <View style={styles.formField}>
          <TextInput
        ...
        {/*<View style={styles.formField}>*/}
        {/*  <CheckBox*/}
        {/*    title="Allow Background Streaming"*/}
        {/*    checked={enableBackgroundStreaming}*/}
        {/*    onPress={this.onEnableBackgroundStreamingChange}*/}
        {/*  />*/}
        {/*</View>*/}
        {/*<View style={styles.formField}>*/}
        {/*  <CheckBox title="Use Authentication" checked={useAuthentication} onPress={this.onUseAuthenticationChange} />*/}
        {/*</View>*/}
        {useAuthentication && (
          <View>
            <TextInput
  ...
  getStateFromProps() {
    ...
    return {
      ...
      streamProps: {
        ...
        configuration: {
          ...this.state.streamProps.configuration,
          host: hostValue,
          licenseKey: licenseValue,
          streamName: streamNameValue,
          parameters: !useAuthentication
            ? ''
            : [['username', usernameValue].join('='), ['password', passwordValue].join('=')].join(';') + ';',
        },
      },
    };
  }
